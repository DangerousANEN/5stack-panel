...
get_oauth_token() {
    ...
    local response; response=$(curl -s -X POST "https://api.tailscale.com/api/v2/oauth/token" \
    ...
    local token; token=$(echo "$response" | grep -o '"access_token":"[^"]*"' | cut -d'"' -f4)
...
create_auth_key() {
    ...
    local response; response=$(curl -s -X POST "${TAILSCALE_API_BASE}/tailnet/-/keys" \
    ...
    local key; key=$(echo "$response" | grep -o '"key":"[^"]*"' | cut -d'"' -f4)
...
update_acl_for_fivestack() {
    ...
    local acl_response; acl_response=$(curl -s -D - -X GET "${TAILSCALE_API_BASE}/tailnet/-/acl" \
        -H "Authorization: Bearer ${access_token}" \
        -H "Accept: application/json")

    local headers; headers=$(echo "$acl_response" | sed '/^\r$/q')
    local body; body=$(echo "$acl_response" | sed '1,/^\r$/d')

    local etag; etag=$(echo "$headers" | grep -i '^etag:' | awk '{print $2}' | tr -d $'\r\n"')
    if [ -z "$etag" ]; then
    ...
    local response; response=$(curl -s -X POST "${TAILSCALE_API_BASE}/tailnet/-/acl" \
        -H "Authorization: Bearer ${access_token}" \
        -H "Content-Type: application/json" \
        -H "If-Match: \"${etag}\"" \
        -d "$updated_acl")
